<main id="threadMain">
  ...
  <div id="threadHeader">
    <button id="backBtn"><a href="/inbox">
  ...
  <div id="threadContains">
    <div id="userCard">
      <div class="userImg"><img src="/assets/IMG/users/<?= $chatsDetails->avatar ?>" id="userAvy"></div><br>
      <div class="username">
        <h5>@<a href="/profil-<?= $chatsDetails->id_users ?>"><?= $chatsDetails->username ?>
          </a></h5>
      </div><br>
    </div>
    <div id="threadContent">
      <div id="contents">
        <p id="contentP"><?= $chatsDetails->content ?></p>
        <div id="timed">
          <p><?= $chatsDetails->publicationDate ?></p>
        </div>
        
        <div class="interact">
          <?php if(!empty($_SESSION['user'])) { ?>
          <button id="likeBtn" name="liked" ><i class="fa-solid fa-heart">Like</i></button>
          <button id="replyBtn" name="replyBtn" quotecontent="<?= $chatsDetails->content ?>"><a href="#comments">répondre</a></button>
          
          <?php if($_SESSION['user']['id'] == $chatsDetails->id_users && ($_SESSION['user']['id_usersRoles'] == 167 || 381)){ ?>
            <button type="submit" name="update" id="editModal" updatecontent=<?= $chatsDetails->content ?> >modifier</button>
...
<hr>
  <div>
    <?php if ($replybackList == 0) { ?>
      <p>Aucune réponse</p>
    <?php } else { ?>
      <?php foreach ($replybackList as $cr) { ?>
        <?php if ($cr['id_users'] == $chatsDetails->id_users) { ?>
...
<?php } else { ?>
    ...
    <fieldset id="userPosting">
      <form action="/chat-<?= $_GET['id'] ?>" method="POST" id="centered">
