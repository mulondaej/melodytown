...
$sql = "SELECT COUNT(*) FROM a8yk4_messages WHERE id = :id_topics";
$req = $this->pdo->prepare($sql);
...
if ($req->fetchColumn() == 0) {
    ...
    throw new Exception("Error: id_topics {$this->id_topics} does not exist in a8yk4_messages.");
}
}
...
$sql = "SELECT COUNT(*) FROM a8yk4_topicreplies WHERE id = :id_topicreplies";
$req = $this->pdo->prepare($sql);
...
if ($req->fetchColumn() == 0) {
    // Message ID does not exist, prevent insertion
    throw new Exception("Error: id_topicreplies {$this->id_topicreplies} does not exist in a8yk4_tex.");
}
}
    /**
    ...
    public function create() {
        $sql = 'INSERT INTO `a8yk4_notifications` (`message`, `link`, `is_read`, `created_at`, `id_topics`, `id_likes`, `id_users`) 
                VALUES (:message, :link, :is_read, NOW(), :id_topics, :id_likes, :id_users)';
    ...
    public function getById()
    {
        ...
        INNER JOIN `a8yk4_messages` AS `m` ON `n`.`id_topics` = `m`.`id`
    ...
    public function getNotifByMessages()
    {
        $sql = 'SELECT n.`id`, n.`link
        FROM `a8yk4_notifications` AS n
        INNER JOIN `a8yk4_messages` AS c ON n.`id_topics` = c.`id`
        WHERE id = :id
        ORDER BY `created_at` DESC ';
        $req = $this->pdo->prepare($sql);
        $req->execute();
